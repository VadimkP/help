...
case "$ans" in
     edo|EDO) echo "Обновляем edo!"
          ...
          cp /data/edo/release_edo/release.$edo/javax.mail-1.5.5.jar /data/edo/release_edo/release.$new_release
                  ...
                  echo "ENV CATALINA_OPTS=\"-Xmx8192m -Xmx8192m\" " >> /data/edo/release_edo/release.$new_release/Dockerfile
                  ...
                  img=$(docker build -t edo:$vers /data/edo/release_edo/release.$new_release --no-cache)
          ...
          down_conteiner=$(docker ps -a | grep tomcat_edo | grep Exited | awk '{ print $1 }')
          docker rm $down_conteiner
		  	  ...
		  	  docker run -d -p8080:8080 -p9999:9999 -v /data/EDOKF_auto/$cms_ver/:/opt/EDOKF/cmsdb_EDOKF_flat/ \
		  	   ...
		  	   --log-opt max-file=20 --name $old_conteiner_name $img

		  echo "Для просмотра логов, пожалуйста запустите в консоле docker log -f $down_conteiner !"
